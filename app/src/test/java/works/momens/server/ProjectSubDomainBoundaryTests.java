 ...
 *
 * <p>{@link ModularityTests}가 검증하지 못하는 범위를 메웁니다. core·task·taskupdate·milestone·blocker는 Spring
 * Modulith named interface라서 application module은 여전히 {@code project} 하나입니다. 그래서 Modulith는 하위 도메인
 * 사이의 의존 방향과 순환을 보지 못합니다. 하위 도메인을 nested application module로 선언하면 방향은 검증되지만 다른 상위 모듈이 참조할 수 없게 되므로
...
class ProjectSubDomainBoundaryTests {
  ...
  void subDomainsDependOnlyInTheAllowedDirection() {
    denyDependency("core", List.of("task", "taskupdate", "milestone", "blocker"));
    denyDependency("blocker", List.of("core", "task", "taskupdate", "milestone"));
    denyDependency("milestone", List.of("task", "taskupdate", "blocker"));
    denyDependency("task", List.of("taskupdate", "blocker"));
    denyDependency("taskupdate", List.of("core", "milestone", "blocker"));
  }
