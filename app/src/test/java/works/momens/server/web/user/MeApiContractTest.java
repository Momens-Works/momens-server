...
@Import({GlobalExceptionHandler.class, MeApiContractTest.ApiVersioningTestConfig.class})
class MeApiContractTest {
  ...
  void missingPrincipalFailsAsUnauthorized() throws Exception {
    mockMvc
        .perform(get("/api/me").header("API-Version", "1"))
  ...
  void invalidPrincipalNameFailsAsInvalidToken() throws Exception {
    ...
    mockMvc
        .perform(get("/api/me").principal(invalidPrincipal).header("API-Version", "1"))
        .andExpect(status().isUnauthorized())
