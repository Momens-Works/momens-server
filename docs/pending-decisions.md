...
| P12 | task 신규 컬럼/테이블 공유 스키마 반영 | 레거시 tasks에 없는 신규 속성은 role 컬럼(MOM-76, CHECK 제약)과 완료기준 신규 테이블 task_checklist_items(MOM-63)로 구현했다(local/test Flyway). 기존 tasks는 바꾸지 않는다. prod 공유 스키마 반영을 컷오버 때 레거시 마이그레이션으로 추가할지 규일과 조율 | 미정 |
