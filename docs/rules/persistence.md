...
레거시가 이미 만든 객체를 `local`/`test`에도 만드는 파일이 23건 있습니다(과거의 `mirror`).
주도권이 넘어와도 **이 파일들의 성격은 바뀌지 않습니다.** prod의 실물은 레거시 DDL이 만든 것이고,
이 파일은 `local`/`test`용 재구성이기 때문입니다. 부트스트랩에서 이 23건은 실행되지 않고 이력에만
심깁니다.

그래서 **"local은 통과하는데 prod에서 깨진다"는 위험이 이 23건에 대해서는 그대로 남습니다.**
