...
`mirror` 헤더가 붙은 마이그레이션은 **레거시 `momens-api`가 이미 소유한 객체를 `local`/`test`에
만드는 파일**입니다. 대개 테이블이지만 확장이나 기존 테이블에 대한 `ALTER`일 수도 있습니다.
prod에서는 공유 DB에 실물이 있고 이 서버의 Flyway가 꺼져 있으므로, 이 파일이 운영 스키마가 되는
일은 없습니다.

그래서 판단 기준은 "얼마나 똑같이 베끼느냐"가 아닙니다. 공유 DB 전환기의 실제 위험은 **local에서는
통과하는데 prod에서 깨지는 것**이고, 미러의 존재 이유는 그 간극을 막는 데 있습니다.
원칙은 양방향입니다.
...
| 항목 | 기준 | 이유 |
| --- | --- | --- |
| 컬럼 이름·타입 | 레거시와 **똑같이**. 예외 없습니다 | prod의 `validate`가 실제로 검사하는 유일한 항목입니다. 갈리면 local은 미러 기준으로 통과하고 prod는 기동에서 죽습니다 |
| 제약 (`NOT NULL`·`CHECK`·`UNIQUE`) | 이 서버가 **쓰는** 테이블이면 레거시와 **똑같이**. 더 느슨해도 더 엄격해도 안 됩니다 | 양방향 원칙이 직접 적용됩니다. UNIQUE는 부분 UNIQUE 인덱스로 걸려 있어도 인덱스가 아니라 제약으로 봅니다 |
...
| 트리거·함수 | **재현하지 않습니다** | prod가 대신 채워 주던 값을 이 서버는 애플리케이션에서 명시적으로 만듭니다(라벨 발급은 `LabelAllocator`). 미러에 트리거를 두면 그 코드가 로컬에서만 우회됩니다 |
| 인덱스 | 필요할 때만. 기본은 만들지 않습니다 | 정확성에 영향이 없습니다. 계획을 테스트로 고정할 때만 필요합니다 |
| PK `DEFAULT uuid_generate_v4()`·미매핑 감사 컬럼 | **무관**. 있어도 없어도 됩니다 | 둘 다 기본값이 있어 없어도 픽스처가 막히지 않고, 있어도 우리가 읽지 않습니다. 거짓 통과도 거짓 실패도 만들지 않습니다 |

몇 가지를 덧붙입니다.

**감사 필드 규칙과의 관계.** 아래 [감사 필드](#감사-필드)의 "모든 테이블에 `created_at`/`updated_at`을
둡니다"는 **이 서버가 설계하는 스키마**에 대한 규칙입니다. 레거시 소유 테이블은 prod에 이미 있어
우리가 정할 여지가 없으므로 그 규칙의 대상이 아니고, 미러는 레거시를 따릅니다. 레거시의 감사
컬럼은 `NOT NULL DEFAULT NOW()`라 미러에 만들든 말든 픽스처가 막히지 않아 위 표에서 "무관"입니다.
`source_refs`가 둘 다 생략하고 `entity_relations`가 `updated_at`을 만든 것이 둘 다 규격 안입니다.
...
- **한 곳이 prod보다 엄격합니다.** `tasks.workspace_id`가 미러에서는 `NOT NULL`이지만 레거시
  ...
  남기고 고치지 않습니다. 레거시가 이 컬럼을 `NOT NULL`로 좁히면 이 항목은 사라집니다.
- **인덱스를 만든 미러가 다섯 있습니다**(`workspaces`·`projects`·`tasks`·`milestones`·memory).
  계획을 고정하는 테스트가 없으므로 새 기준에서는 만들 필요가 없었던 것들입니다. 무해하고 이미
  적용된 파일이라 지우지 않습니다. 특히 **memory 미러 주석의 "미러의 값은 prod를 얼마나 그대로
  재현하느냐에 있다"는 근거는 이 절이 대체합니다.** 파일은 checksum 때문에 고치지 않으니, 그
  주석을 보고 재현 충실도를 기준으로 되돌아가지 않도록 여기에 적어 둡니다.
