...
- 어떤 일에 걸린 시간은 언제나 `duration`입니다. `latency`·`elapsed`·`response.time`을 섞어 쓰면
  ...
  `age`는 되도록 값의 출처와 같은 시계로 계산합니다. 인스턴스 간 시계 편차가 그대로 지표 편차가
  됩니다. 시계를 섞을 수밖에 없으면(예: 애플리케이션이 쓴 시각을 DB 시계로 빼는 경우) 음수를 0에서
  자르고, 어느 값이 어느 시계에서 오는지 계측하는 자리에 남깁니다.
...
- **Micrometer 표준 바인더가 내는 이름은 이 규칙 밖입니다**(예: `ExecutorServiceMetrics`의
  `executor.*`, `jvm.*`). 같은 의미의 지표를 우리 이름으로 다시 만들면 이름만 우리 것이 되고 의미는
  같아지므로, 표준 바인더가 있으면 그것을 씁니다.
