...
- 워크스페이스 권한 부족으로 `AUTH_FORBIDDEN`을 반환할 때 포함하는 `details`는 [서버 명세 > API 응답과 에러 코드](../spec/api-response-error-codes.md)의 「권한 details」 절을 따릅니다. `web` 모듈에서는 `WorkspaceAccessChecker`가 이 형식을 관리하며, 다른 곳에서 권한을 직접 판정하지 않는지는 `WebWorkspaceAccessBoundaryTests`에서 검증합니다.
