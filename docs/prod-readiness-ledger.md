...
| **MOM-0909 부트스트랩 DDL 선행 권한** | `승인 대기` | 부트스트랩이 성립하려면 관리자 조치 다섯 가지가 먼저 필요하고 **순서가 있습니다** — `GRANT momens_server TO postgres WITH SET TRUE` 를 가장 먼저 실행해야 소유권 이전이 성립합니다(비-superuser 는 대상 role 로 `SET ROLE` 할 수 있어야 `ALTER TABLE ... OWNER TO` 를 할 수 있습니다). 그 GRANT 자체는 `momens_server` 에 대한 `ADMIN OPTION` 을 요구하므로, `postgres` 가 그 role 을 만들지 않았다면 통과하지 못합니다(설계 8절 3단계의 진단 쿼리). `GRANT USAGE ON SCHEMA extensions TO momens_server` + `ALTER ROLE momens_server SET search_path = "$user", public, extensions` (Supabase는 `uuid-ossp`를 `extensions`에 두는데 실행 집합 2건이 한정 없이 `uuid_generate_v4()`를 호출합니다. **둘 다** 필요하며, 파일 수정은 체크섬 때문에 불가능합니다), `GRANT SELECT, INSERT, UPDATE, DELETE ON tasks TO momens_server` (기존 GRANT 18건은 `tasks`를 의도적으로 뺐습니다 — 소유권 이전으로 대체하려던 판단이라 런타임 DML이 이전 성공에만 매달려 있습니다. **소유권 이전보다 먼저** 실행합니다), `ALTER TABLE tasks OWNER TO momens_server` (실행 집합이 `tasks`를 `ALTER` 하고, `ALTER TABLE`은 GRANT 대상이 아니라 소유자 권한입니다) 와 `GRANT REFERENCES ON users TO momens_server` (`V20260810090000`이 `users(id)`를 참조하는 FK를 걸고, `REFERENCES`는 DML 권한에 포함되지 않습니다), `GRANT momens_server TO postgres WITH SET TRUE` (심기 SQL이 이력 테이블 소유권을 넘기는데, Supabase의 `postgres`는 superuser가 아니라 대상 role로 `SET ROLE`할 수 있어야 합니다. `CREATEROLE`의 자동 멤버십은 `set=false`라 쓸 수 없습니다). 없으면 각각 `must be owner of table tasks` / `permission denied for table users` / 심기 자체가 `must be able to SET ROLE`로 죽습니다 — `scripts/prod-twin` 리허설이 실패로 재현했습니다. 소유권 이전은 되돌리기 번거로워 승인 대기 중이고, 나머지 셋은 GRANT 한 줄씩입니다. 소유권을 나중에 되돌린다면 `tasks` DML GRANT를 함께 재발급해야 합니다 — `ALTER TABLE ... OWNER TO`가 소유자를 grantee로 하는 ACL을 함께 가져가고, 이 손실은 `validate`가 보지 않아 기동에서 잡히지 않습니다(설계 7절). 세 번째를 적용하지 않을 거라면 대안은 관리자가 `ALTER TABLE flyway_schema_history OWNER TO momens_server`를 직접 실행하는 것입니다. **부트스트랩이 적용되면 이 행을 제거합니다.** |
