...
| --- | --- | --- |
| **MOM-0909 부트스트랩 적용 전 `main` 릴리스 금지** | `required` | 스키마 릴리스 게이트를 MOM-0910에서 폐지했습니다(교착 해소, [설계 6절](design/prod-schema-ownership-transfer.md)). 그 게이트가 막던 것 중 하나는 크로스 리포 의무가 아니라 **기동 실패**였고, 그 보호가 지금은 없습니다. 부트스트랩 적용 전에 `develop` → `main` 릴리스가 나가면 미반영 16건 때문에 `ddl-auto=validate`에서 기동에 실패하고, `startupProbe` 150초 뒤 `rollout status --timeout=300s`가 깨집니다. `deploy-service.sh`에 `rollout undo`가 없어 Ready 되지 못한 ReplicaSet이 수동 정리 전까지 남습니다. 사용자 영향은 없습니다(`replicas: 1` + `maxUnavailable=0`으로 옛 Pod 유지, FE 미cutover). 릴리스 PR은 미리 열어 CI·리뷰를 끝내되 head SHA·tree hash·main base SHA를 고정하고, 그 내용으로 `--verify`·`--generate`·이력 심기를 수행합니다. 토글 PR 머지 직전에 같은 head·tree·base와 그 head의 필수 CI를 다시 확인하고, 토글 머지와 `--match-head-commit` 릴리스 머지를 같은 작업 창에서 연속 수행합니다. 그 사이 다른 `main` 머지와 수동 dispatch는 금지합니다. **부트스트랩이 적용되면 이 행을 제거합니다.** |
| **MOM-0909 접속 대상을 Supabase로 교체** | `required` | `MOMENS_SERVER_DATABASE_URL`·`_USERNAME`·`_PASSWORD`(`k8s` 리포 GitHub Secret)가 아직 옛 Neon을 가리킵니다. 위 선언 구간이 이 셋을 `applied`로 두는 것은 **주입되고 있다**는 뜻이지 대상이 맞다는 뜻이 아닙니다 — 값의 내용은 CI가 볼 수 없어 여기서 사람이 추적합니다. 교체 전에 토글을 켜면 소유권 이전·심기는 Supabase에서 일어나고 Flyway는 Neon에서 42건을 처음부터 실행하려 합니다. **주소는 direct여야 하고 판별은 포트가 아니라 host입니다** — session pooler도 `5432`를 쓰고(`aws-0-<region>.pooler.supabase.com`, username에 `.<ref>`), direct는 `db.<ref>.supabase.co`입니다. 교체 후 앱 자격증명으로 `current_database()`·`current_user`·`inet_server_addr()`·`inet_server_port()`와 `schema_migrations` 존재를 확인하기 전에는 ConfigMap 토글을 켜지 않습니다([설계 8절 0단계](design/prod-schema-ownership-transfer.md)). **확인이 끝나면 이 행을 제거합니다.** |
| **MOM-0909 부트스트랩 DDL 선행 권한** | `승인 대기` | 부트스트랩이 성립하려면 관리자 조치 세 가지가 먼저 필요하고 **순서가 있습니다.** ① `GRANT momens_server TO postgres WITH SET TRUE` — 비-superuser 인 Supabase `postgres` 가 `ALTER TABLE ... OWNER TO` 를 하려면 대상 role 로 `SET ROLE` 할 수 있어야 합니다. 소유권 이전이 여기에 의존하므로 가장 먼저 실행합니다(심기는 DML 부여라 의존하지 않습니다 — 아래 이력 테이블 권한 행 참조)(prod 실측: `admin_option = true` 라 통과합니다). ② `scripts/prod-ownership-transfer.sh --generate` 로 만든 소유권 일괄 이전 SQL — 서버가 마이그레이션 파일을 갖는 레거시 테이블 20개입니다. `ALTER TABLE` 은 GRANT 체계 밖이라 `GRANT ALL PRIVILEGES` 로도 얻을 수 없고, ADR-0019 의 최종 상태이므로 한 번에 받습니다. 생성물이 이전과 `GRANT ... TO postgres` 재발급을 짝으로 묶고, 재발급을 `SET LOCAL ROLE momens_server` 로 감쌉니다(이전 직후 창구는 소유자가 아니라 GRANT 를 할 수 없습니다) — **재발급이 빠지면 레거시가 끊깁니다.** ③ `GRANT USAGE ON SCHEMA extensions TO momens_server` + `ALTER ROLE momens_server SET search_path = "$user", public, extensions` — Supabase 는 `uuid-ossp` 를 `extensions` 에 두는데 실행 집합 2건이 한정 없이 `uuid_generate_v4()` 를 호출합니다. **둘 다** 필요하며 파일 수정은 체크섬 때문에 불가능합니다. 셋 다 `scripts/prod-twin` 리허설이 없을 때의 실패를 재현했습니다. 소유권을 나중에 되돌린다면 20개 전부에 DML 재발급이 함께 가야 합니다(설계 7절). **부트스트랩이 적용되면 이 행을 제거합니다.** |
| **MOM-0909 이력 테이블 권한** | `스크립트가 처리` | 심기 후 `momens_server` 가 `flyway_schema_history` 에 DML 권한을 가져야 합니다. Postgres 는 테이블을 만든 role 을 소유자로 두는데 실행 창구인 Supabase SQL Editor 는 `postgres` 세션이고, `momens_server` 로 psql 을 여는 선택지는 없습니다(비밀번호를 아는 사람이 없습니다). `--generate` 생성물이 트랜잭션 안에서 `GRANT SELECT, INSERT, UPDATE, DELETE ON flyway_schema_history TO momens_server` 를 수행하므로 **생성물을 손대지 않고 그대로 실행하면 됩니다.** 소유권 이전이 아닌 이유는 관리자가 소유권 일괄 이전 직후 `SET` 능력을 다시 끄기 때문입니다(2026-08-27). 그 줄이 빠지면 심기는 성공하고 다음 기동이 `permission denied for table flyway_schema_history` 로 죽습니다 — 심기 시점에는 신호가 없습니다. `prod-flyway-bootstrap-verify-test.sh` 가 CI 에서 그 줄을 지킵니다. **부트스트랩이 적용되면 이 행을 제거합니다.** |
