...
- 기본 project id는 접근 가능한 project 중 가장 최근에 만든 것이다(2026-07-04 가결정, 기획
  확인 후 확정).
...
- 태스크 카드에는 제목, roles, priority, 관련자료 개수를 표현할 수 있어야 한다.
...
- 응답은 태스크 기본 정보, roles, 담당자, 우선순위, 목적, 체크리스트, 관련자료 축약본, 열린 질문,
  다음 액션을 표현할 수 있어야 한다.
...
- MVP 생성 범위는 `title`, `roles`, `priority`다.
...
- MVP 수정 범위는 제목, roles, 담당자, 우선순위, 목적, 완료기준이다.
...
- bootstrap의 기본 project 선정 규칙과 project 0개 응답 정책의 확정(R-READ-000의 2026-07-04
  가결정안으로 구현했고, 기획 확인 후 확정한다)
