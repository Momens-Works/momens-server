 ...
 * outbox·signal·workspace public API는 다른 모듈 소유라 mock으로 둡니다.
 */
@DataJpaTest
...
class SignalCreatedDeliveryMaterializerIntegrationTest extends AbstractPostgresIntegrationTest {
  ...
  void materializesDeliveriesForActiveInstallations() {
    PushInstallation activeA = saveInstallation("fid-a", MEMBER_A, "token-a", true);
    PushInstallation inactiveA = saveInstallation("fid-a2", MEMBER_A, "token-a2", false);
    PushInstallation activeB = saveInstallation("fid-b", MEMBER_B, "token-b", true);
    ...
    assertThat(deliveries)
        ...
        .containsExactlyInAnyOrder(activeA.getId(), activeB.getId())
        .doesNotContain(inactiveA.getId());
    assertThat(deliveries)
        .allSatisfy(
            delivery -> {
  ...
  void replayDoesNotDuplicateDeliveries() {
    saveInstallation("fid-a", MEMBER_A, "token-a", true);
    stubSignalCreatedEvent();
