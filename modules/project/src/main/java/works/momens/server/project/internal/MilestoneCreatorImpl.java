 ...
 *
 * <p>{@link ProjectCreatorImpl}과 달리 라벨과 집계 컬럼은 다루지 않습니다. {@code milestones} 테이블에는 해당 컬럼이 없습니다. 상태는
 * 요청으로 받지 않고 항상 {@code planned}로 저장합니다. 레거시의 마일스톤 생성 요청도 상태를 입력받지 않습니다.
 */
@Service
...
class MilestoneCreatorImpl implements MilestoneCreator {
  ...
  private List<UUID> resolveOwnerUserIds(CreateMilestoneCommand command) {
    ...
    List<UUID> projectOwnerUserIds =
        projectOwnerRepository
            .findByProjectIdInOrderByCreatedAtAscOwnerUserIdAsc(List.of(command.projectId()))
            .stream()
