...
이 쌍둥이는 **세션이 유지되는 접속**을 전제합니다. prod 는 session pooler
(`aws-0-ap-southeast-1.pooler.supabase.com:5432`)를 쓰고 세션이 유지되므로 그 전제가 맞습니다.
접속 정보를 교체할 때 트랜잭션 pooler(`:6543`) 주소로 바뀌면 Flyway 의 세션 단위 잠금과
`init-sqls` 가 성립하지 않아 여기서 얻은 락 결과가 무효가 됩니다.
