...
generate() {
...

    {
        ...
        echo "    -- Supabase SQL Editor 는 WARNING 을 눈에 띄게 보여주지 않아서 사실상 조용히"
        echo "    -- 넘어가고, 그러면 19 개만 넘어간 부분 상태가 COMMIT 된다. 운영자는 끝났다고"
        echo "    -- 믿는데 부트스트랩이 must be owner of table 로 죽는다."
        echo "    IF absent <> '' THEN"
        echo "        RAISE EXCEPTION '대상에 없는 테이블이 있습니다:%', absent;"
        echo "    END IF;"
        echo "END \$\$;"
        echo ""
