SELECT name, slug, description FROM workspaces WHERE slug = 'momens-create';
SELECT wm.role FROM workspace_members wm JOIN workspaces w ON w.id = wm.workspace_id
WHERE w.slug = 'momens-create' ORDER BY wm.user_id;
...
FROM projects p JOIN workspaces w ON w.id = p.workspace_id WHERE w.slug = 'momens-create';
...
WHERE w.slug = 'momens-create' ORDER BY po.owner_user_id;
...
WHERE w.slug = 'momens-create' ORDER BY m.label;
SELECT ls.label_prefix, ls.next_value FROM workspace_label_sequences ls
JOIN workspaces w ON w.id = ls.workspace_id WHERE w.slug = 'momens-create' ORDER BY ls.label_prefix;
