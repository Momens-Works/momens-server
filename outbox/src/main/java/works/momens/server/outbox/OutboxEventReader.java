 ...
 * 스캔을 멈춘다(prefix-cap). 중간을 걸러내지 않으므로 commit된 event는 최대 안전 지연만큼 늦어질 뿐 건너뛰어지지 않는다. 기준 시각은 생산자와 같은 DB
 * 시계({@code NOW()})라 앱 서버 시계 차이의 영향을 받지 않는다. 소비 상태(watermark)는 consumer가 자기 모듈에서 관리하고, 이 모듈은 조회만
 * 제공한다.
 */
public interface OutboxEventReader {
   ...
   * 멈춘다. 반환 목록의 마지막 id까지 watermark를 전진시켜도 event를 건너뛰지 않는다.
   */
